...
var Utils = {
  ...
  getIdealType: function( $el ) {
    ...
    return (
      /(text|password|email|number|search|url|tel|textarea|date|phone)/.test( type ) && 'text' ||
      /file/.test( type ) && 'file' ||
  ...
  makeInput: function( name, value, type, list, placeholder ) {
    ...
    // Text & file
    if ( /^(text|password|email|phone|number|search|url|tel|file|hidden|date)$/.test(type) )
      markup = '<input '+
        'type="'+ type +'" '+
        'id="'+ name +'" '+
        'name="'+ name +'" '+
...
$.fn.idealSelect = function () {
  ...
  return this.each(function () {
        ...
        var keys = {
          ...
          'default': function () { // Letter
            ...
            $matches = idealSelect.items
              .filter(function () {
                return /^\w+$/i.test( letter ) && // not allow modifier keys (
													// ctrl, cmd, meta, super...
													// )
    ...
    /**
	 * @namespace Holds all events of custom select for "menu mode" and "list
	 *            mode"
	 * @memberOf $.fn.toCustomSelect
	 */
    var events = {
...
$.fn.idealRadioCheck = function() {
  ...
  return this.each(function() {
    ...
    $this.css({ position: 'absolute', left: '-9999px' }) // hide by shifting
															// left
    ...
    if ( userFilters ) {
      ...
      $.each( userFilters, function( i, filter ) {
        ...
        if ( theFilter ) {
          ...
          // AJAX
          if ( filter === 'ajax' ) {
            showOrHideError( error, false ) // set invalid till response comes
											// back
            $error.hide()
            if ( e.type === 'keyup' ) {
              theFilter.regex( inputData, value, showOrHideError ) // runs the
																	// ajax
																	// callback
            } else {
          ...
          // All other filters
            ...
            error = customError || theFilter.error // assign error after
													// calling regex()
